 ...
 * @since  1.0.0
 */
class SlaViewSlaActivityType extends HtmlView
{
	/**
	 * The JForm object
	 *
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = JHelperContent::getActions('com_sla', 'slaactivitytype', $this->item->id);
	...
	protected function addToolbar()
	{
		...
		$this->slaHelper->addSubmenu('slaactivitytypes');
		...
		if ($layout != "default")
		{
			...
			JToolbarHelper::title(
				JText::_('COM_SLA_PAGE_' . ($checkedOut ? 'VIEW_SLA_ACTIVITY_TYPE' : ($isNew ? 'ADD_SLA_ACTIVITY_TYPE' : 'EDIT_SLA_ACTIVITY_TYPE'))),
				'pencil-2 slaactivitytype-add'
			);

			if ($isNew)
			{
				JToolBarHelper::apply('slaactivitytype.apply', 'JTOOLBAR_APPLY');
				JToolbarHelper::save('slaactivitytype.save');
				JToolbarHelper::cancel('slaactivitytype.cancel');
			}
			else
			{
				...
				JToolbarHelper::cancel('slaactivitytype.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			JToolbarHelper::title(
				JText::_('COM_SLA_PAGE_VIEW_SLA_ACTIVITY_TYPE')
			);

			$app = Factory::getApplication();

			JLoader::import('administrator.components.com_sla.helpers.sla', JPATH_SITE);
			SlaHelper::addSubmenu('slaactivitytype');
