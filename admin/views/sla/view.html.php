 ...
 * @since  1.0.0
 */
class SlaViewSla extends HtmlView
{
	/**
	 * The JForm object
	 *
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = JHelperContent::getActions('com_sla', 'sla', $this->item->id);
	...
	protected function addToolbar()
	{
		...
		$this->slaHelper->addSubmenu('slas');
		...
		if ($layout != "default")
		{
			...
			JToolbarHelper::title(
				JText::_('COM_SLA_PAGE_' . ($checkedOut ? 'VIEW_SLA' : ($isNew ? 'ADD_SLA' : 'EDIT_SLA'))),
				'pencil-2 sla-add'
			);

			if ($isNew)
			{
				JToolBarHelper::apply('sla.apply', 'JTOOLBAR_APPLY');
				JToolbarHelper::save('sla.save');
				JToolbarHelper::cancel('sla.cancel');
			}
			else
			{
				...
				JToolbarHelper::cancel('sla.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			JToolbarHelper::title(
				JText::_('COM_SLA_PAGE_VIEW_SLA')
			);

			$app = Factory::getApplication();

			JLoader::import('administrator.components.com_sla.helpers.sla', JPATH_SITE);
			SlaHelper::addSubmenu('sla');
