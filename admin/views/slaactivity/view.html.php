	...
	protected function addToolbar()
	{
		...
		if ($layout != "default")
		{
			...
			JToolbarHelper::title(
				JText::_('COM_SLA_PAGE_' . ($checkedOut ? 'VIEW_SLA_ACTIVITY' : ($isNew ? 'ADD_SLA_ACTIVITY' : 'EDIT_SLA_ACTIVITY'))),
				'pencil-2 slaactivity-add'
			);

			if ($isNew)
			{
				JToolBarHelper::apply('slaactivity.apply', 'JTOOLBAR_APPLY');
				JToolbarHelper::save('slaactivity.save');
				JToolbarHelper::cancel('slaactivity.cancel');
			}
			else
			{
				...
				JToolbarHelper::cancel('slaactivity.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			JToolbarHelper::title(
				JText::_('COM_SLA_PAGE_VIEW_SLA_ACTIVITY')
			);
